...
const tntActionMap = TNT_ACTIONS.reduce((map, enumName) => {
...
_.map(tntActionMap, action => _.set(action, 'xform', x => x))
tntActionMap.createRecord.xform = valueXform
tntActionMap.createRecordType.xform = schemaXform
tntActionMap.updateProperties.xform = valueXform
...
const tntActionMethods = _.reduce(tntActionMap, (methods, value, key) => {
  return _.set(methods, key, _.partial(encode, key))
}, {})

// Add enums on an action by action basis
tntActionMethods.createRecord.enum = PropertySchema.DataType
tntActionMethods.createRecordType.enum = PropertySchema.DataType
tntActionMethods.updateProperties.enum = PropertySchema.DataType
tntActionMethods.createProposal.enum = Proposal.Role
tntActionMethods.answerProposal.enum = tntActionMap.answerProposal.proto.Response

module.exports = _.assign({
  encode,
  FLOAT_PRECISION: 1000000
}, tntActionMethods, pikeActionMethods)
